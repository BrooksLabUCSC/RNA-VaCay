...
library(ggplot2)
library(reshape2)
setwd("/Users/jonakutagawa/git/RNA-VaCay/backup_summary")

# Change these directories to fit your file system to where outDir was for
# post-process-wrap.sh
platDir <- "/Users/jonakutagawa/git/RNA-VaCay/backup_summary/pon-platy-summ/"
varDir <- "/Users/jonakutagawa/git/RNA-VaCay/backup_summary/pon-var-summ/"
mutDir <- "/Users/jonakutagawa/git/RNA-VaCay/backup_summary/pon-mut-summ/"
#

platCounts <- read.delim(paste(platDir, "countBreakdown.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
varCounts <- read.delim(paste(varDir, "countBreakdown.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
mutCounts <- read.delim(paste(mutDir, "countBreakdown.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)

# total number of variants in whitelist
varTotal <- read.delim(paste(varDir, "totalBreakdown.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)

platSens <- read.delim(paste(platDir, "sensitivityFPKM.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
varSens <- read.delim(paste(varDir, "sensitivityFPKM.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)
mutSens <- read.delim(paste(mutDir, "sensitivityFPKM.txt", sep = ""), sep = "\t",
  header = TRUE, stringsAsFactors = FALSE)

platCovFound <- read.delim(paste(platDir, "found.cov.txt", sep = ""), sep = "\t",
  header = FALSE, stringsAsFactors = FALSE)
platCovNotFound <- read.delim(paste(platDir, "notFound.cov.txt", sep = ""), sep = "\t",
  header = FALSE, stringsAsFactors = FALSE)

varCovFound <- read.delim(paste(varDir, "found.cov.txt", sep = ""), sep = "\t", header = FALSE,
  stringsAsFactors = FALSE)
varCovNotFound <- read.delim(paste(varDir, "notFound.cov.txt", sep = ""), sep = "\t",
  header = FALSE, stringsAsFactors = FALSE)

mutCovFound <- read.delim(paste(mutDir, "found.cov.txt", sep = ""), sep = "\t", header = FALSE,
  stringsAsFactors = FALSE)
mutCovNotFound <- read.delim(paste(mutDir, "notFound.cov.txt", sep = ""), sep = "\t",
  header = FALSE, stringsAsFactors = FALSE)
...
varTotal <- varTotal %>% mutate_all(funs(ifelse(is.na(.), 0, .))) %>% select(-Donor.ID,
  -Total)
print("varCounts")
print(head(varCounts))
## sum down each column (variant type) to get total variants in whitelist for all
## bams
...
## to print total num of variants in same order as varTotal and add column to
## retain tool type before bind
...
# sumBreakdown$Tool <- factor(sumBreakdown$Tool, levels = c('VarDict',
# 'Platypus', 'MuTect2'))
maxSum = max(sumBreakdown$Sum)
print(sumBreakdown)
######## Plot sum with IGR, 5'Flank, and Introns ##########
withIGRHisto <- ggplot(sumBreakdown, aes(x = Variant.Type, y = Sum, fill = Tool)) +
  geom_bar(stat = "identity", position = "dodge") + coord_flip() + geom_text(aes(label = paste(Sum,
  " of ", Total)), hjust = -0.3, size = 3, position = position_dodge(width = 1)) +
  theme_classic() + guides(fill = guide_legend(reverse = TRUE)) + scale_x_discrete(limits = positions) +
  scale_y_continuous("Count", expand = c(0, 0), limits = c(0, maxSum + 6000))
ggsave(withIGRHisto, filename = "HistoWithIGR_4.jpg")

########## Plot Sum without IGR, 5'Flank, and Introns ###########
sumBreakdown <- sumBreakdown %>% filter(Variant.Type != "IGR" & Variant.Type != "Intron" &
  Variant.Type != "X5.Flank")
print(sumBreakdown)
# sumBreakdown <- sumBreakdown %>% gather(Variant.Type, Mean) %>%
# arrange(desc(Sum))
print(positions)
...
maxSum = max(sumBreakdown$Sum)
print("done with IGR")
noIGRHisto <- ggplot(sumBreakdown, aes(x = Variant.Type, y = Sum, fill = Tool, order = -as.numeric(Tool))) +
  geom_bar(stat = "identity", position = "dodge") + coord_flip() + geom_text(aes(label = paste(Sum,
  " of ", Total)), hjust = -0.3, size = 3, position = position_dodge(width = 1)) +
  theme_classic() + guides(fill = guide_legend(reverse = TRUE)) + scale_x_discrete(limits = positions) +
  scale_y_continuous("Count", expand = c(0, 0), limits = c(0, maxSum + 6000))
...
platsensFPKM <- as.data.frame(platsensFPKM %>% group_by(bin) %>% mutate(Sensitivity = foundAll/(totalAll)) %>%
  mutate_all(funs(ifelse(is.na(.), 0, .))))
platsensFPKM <- platsensFPKM %>% mutate(Tool = "Platypus")
varsensFPKM <- as.data.frame(varsensFPKM %>% group_by(bin) %>% mutate(Sensitivity = foundAll/(totalAll)) %>%
  mutate_all(funs(ifelse(is.na(.), 0, .))))
varsensFPKM <- varsensFPKM %>% mutate(Tool = "VarDict")
mutsensFPKM <- as.data.frame(mutsensFPKM %>% group_by(bin) %>% mutate(Sensitivity = foundAll/(totalAll)) %>%
  mutate_all(funs(ifelse(is.na(.), 0, .))))
...
hist_top <- ggplot(platsensFPKM, aes(x = bin, y = totalAll)) + geom_bar(stat = "identity") +
  theme_classic() + theme(axis.text.x=element_text(size=rel(1.25)),axis.text.y=element_text(size=rel(1.25))) + scale_x_continuous(limits = c(-6, 3.25), breaks = seq(-6, 3,
  by = 1)) + scale_y_continuous(trans = "log2", breaks = c(5, 30, 250, 2050))
...
sensPlot <- ggplot(sensFPKM, aes(x = bin, y = Sensitivity, color = Tool, group = Tool)) +
  geom_point() + geom_line() + theme_classic() + theme(legend.position = c(0.075,
  0.8), legend.title = element_blank()) + theme(axis.text.x=element_text(size=rel(1.25)),axis.text.y=element_text(size=rel(1.25))) + scale_x_continuous(limits = c(-6, 3.25),
  breaks = seq(-6, 3, by = 1)) + scale_y_continuous(limits = c(0, 1), breaks = seq(0,
  1, by = 0.1))
sensPlot <- sensPlot + labs(x = expression(log["2"](FPKM)), y = "Sensitivity")
...
dev.off()
# ggsave(totalPlot, filename='sensitivityFPKM.jpg', width=10, height=6,
# units='in', dpi=300)




######################## Plot Coverage vs Sensitivity ################## concat coverage files for each
######################## tool ##
...
print(head(covFound))
## set bins by total coverage ## set max read coverage bin to show on plot (goes
## >15,000 but no variants in most bins)
binMax <- 450  #max(covNotFound$V7)
print(binMax)
covFound$bin <- cut(covFound$V7, breaks = seq(0, binMax + 25, by = 25), labels = seq(0,
  binMax, by = 25))
covNotFound$bin <- cut(covNotFound$V7, breaks = seq(0, binMax + 25, by = 25), labels = seq(0,
  binMax, by = 25))
...
totCov <- as.data.frame(totCov %>% group_by(Tool, bin) %>% mutate(total = as.numeric(found +
  notFound)))
print("totCov")
print(head(totCov))

## calculate sensitivity and add as column for each bin
totCov <- as.data.frame(totCov %>% group_by(Tool, bin) %>% mutate(sensitivity = (as.numeric(found)/total)) %>%
  mutate_all(funs(ifelse(is.na(.), 0, .))))
print(totCov)

## open jpg file for writing
jpeg(filename = "sensitivityCoverage_4.jpg", width = 10, height = 6, units = "in",
  res = 300)
maxCount = max(totCov$total)
histCov <- totCov %>% distinct(totCov, total, .keep_all = TRUE)

## plot histogram of known variants in each bin
hist_top <- ggplot(histCov, aes(x = bin, y = total)) + geom_bar(stat = "identity",
  position = position_dodge(width = 1)) + theme_classic() + theme(axis.text.x=element_text(size=rel(1.25)),axis.text.y=element_text(size=rel(1.25))) + scale_y_continuous(trans = "log2",
  breaks = c(5, 30, 250, 2050)) + scale_x_discrete(breaks = seq(0, binMax, by = 25))  #+ scale_y_continuous(limits=c(0, maxCount=50), trans='log2')
hist_top <- hist_top + labs(y = "# Known Variants", x = "")
## plot sensitivity vs read coverage
sensPlot <- ggplot(totCov, aes(x = bin, y = sensitivity, color = Tool, group = Tool)) +
  geom_point() + geom_line() + theme_classic() + theme(axis.text.x=element_text(size=rel(1.25)),axis.text.y=element_text(size=rel(1.25))) + theme(legend.position = c(0.075,
  0.8), legend.title = element_blank()) + scale_x_discrete(breaks = seq(0, binMax +
  25, by = 25)) + scale_y_continuous(limits = seq(0, 1), breaks = seq(0, 1, by = 0.1))
